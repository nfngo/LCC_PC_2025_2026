...
public class ScoreboardScreen {
    ...
    public void draw() {
        ...
        p.text("SCOREBOARD", centerX, startY + 40);

        // Subtítulo
        p.textSize(24);
        p.text("TOP 10 SCORES", centerX, startY + 100);
        ...
        if(!message.isEmpty()) {
            p.text(message, centerX, tableTop + 25);
        }

        for (int i = 0; i < Math.min(scores.size(), 10); i++) {
            int rank = i + 1;
            ParseScore s = scores.get(rank);
            ...
            p.fill(0);
            p.text(rank, rankX, y);
            p.text(s.username(), centerX, y);
            p.text(s.score(), scoreX, y);
        }
